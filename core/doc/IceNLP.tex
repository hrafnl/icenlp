...
Evaluation has shown that \textit{IceTagger} achieves higher accuracy than the best performing data-driven tagger when tested using the same test corpora and the same ratio of unknown words \citep{lof08,hel04}.
...
Furthermore, an accuracy of 92.51\% is achieved by using \textit{TriTagger} as a word class tagger during initial disambiguation and then using \textit{IceTagger} to disambiguate tags that are consistent with the chosen word class \citep{lof09b}.
...
Furthermore, if neither the -i/-o parameters nor the -f parameter are provided, \textit{IceTagger} reads from standard input and writes to standard output. For example, the following Linux command can be used to make \textit{IceTagger} tag the string ``Ég á stóran hund'' (and write the output to the screen): 
\begin{verbatim}
...
Furthermore, if the -i/-o parameters are not provided, \textit{TriTagger} reads from standard input and writes to standard output. For example, the following Linux command can be used to make \textit{TriTagger} tag the string ``Ég á stóran hund'' (and write the output to the screen): 
\begin{verbatim}
