...
The toolkit consists of a tokeniser, the morphological analyser \emph{IceMorphy}, the linguistic rule-based tagger \emph{IceTagger}, the trigram tagger \emph{TriTagger}, the shallow parser \emph{IceParser}, the lemmatiser \emph{Lemmald}, and the named entity recogniser \emph{IceNER}.
...
\emph{Lemmald}\footnote{\emph{Lemmald}, as a stand-alone program, can be downloaded from \url{http://www.hi.is/~antoni/lemmald/}.} is a mixed method lemmatiser for Icelandic.
...
The {\bf lib} directory contains the \emph{Lemmatizer.jar} file.  This file consists of all the .class files needed to run \emph{Lemmald}. 
...
It which achieves good performance by relying on \emph{IceTagger} for tagging and the \emph{IFD} corpus for training.
...
\label{sec:tok}
The tokeniser application is used for tokenising input files and converting between different file formats.
...
Furthermore, if the -i/-o parameters are not provided, the tokenizer reads from standard input and writes to standard output. In that case, inputFormat=3 and outputFormat=1. For example, the following Linux command can be used to tokenize the string ``Ég á stóran hund. Sá er a.m.k. 10 kíló.'' (and write the output to the screen): 
\begin{verbatim}
...
\item \emph{LEMMATIZER\_FILE}: If IceTagger should output the lemma, in addition to word and tag, then this is used to specify the location of the settings file for the lemmtizer. 
...
%\label{sec:messageProtocol}
%\emph{IceTagger} can be used as a server accepting messages containing text to be tagged.
%In this case, \emph{IceTagger} waits for a message, reads its content, extracts the text from it, tokenises and tags the sentence and returns back a tagged version of the sentence.

%The message and communication protocol used is based on the AIR publish-subscribe protocol, built especially for A.I. research (see www.mindmakers.org).

%A precise description of how to use \emph{IceTagger} in this case will follow in a later version of this User Guide.
